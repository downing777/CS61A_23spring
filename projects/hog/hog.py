...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        outcomes.append(dice())  # you can't write a = a.append(x)
    if 1 in outcomes:
        return 1
    else:
        return sum(outcomes)
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    dice:            A function that simulates a single dice roll outcome.
    """
    # compute the points you will get in this turn
...
def perfect_square(n):
    assert n >= 0, 'input must be an non-negative integer'
...
def play(strategy0, strategy1, update,
    ...
    strategy0: The strategy for player0. #A function that returns the number of dice a player will roll, take in the current scores and opponent's scores
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    def always_strategy(score, opponent_score):
        return n
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # ergodic method
    # BEGIN PROBLEM 7
    compare = strategy(0, 0)
    flag = True
    for score in range(goal):
        for opponent_score in range(goal):
            if strategy(score, opponent_score) != compare:
                flag = False
                break
    return flag
...
def make_averaged(original_function, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def return_average(*arg): # different original_functions have different input format
        outcomes = []
        for i in range(total_samples):
            outcomes.append(original_function(*arg))
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    pigtail = tail_points(opponent_score)
    if pigtail >= threshold:
        return 0
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    pig_tail_gain = square_update(0, score, opponent_score) - score
    if pig_tail_gain >= threshold:
        return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
