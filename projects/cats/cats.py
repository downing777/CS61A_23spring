...
def accuracy(typed, source):
    ...
    if len(typed_words) == 0 and len(source_words) == 0:
        return 100.0
    elif len(typed_words) == 0 or len(source_words) == 0:  # be careful about the order it computes the operator '&' is prior to '=='
        return 0.0
        ...
        for i in range(len(typed_words)):
            if typed_words[i] == source_words[i]:
                correct_num += 1
        return float(100 * correct_num / len(typed_words))
    else:
        correct_num = 0
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    return float(len(typed) * (60 / elapsed) / 5)
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    difference = [diff_function(typed_word, i, limit) for i in word_list]
    if typed_word in word_list:
        return typed_word
    elif min(difference) > limit:
        return typed_word
    else:
...
def feline_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    if limit == 0 and typed != source: # break the substitude when reach the limit
        return 1
    elif limit == 0 and typed == source:
        return 0
    elif len(typed) == 0 or len(source) == 0:
        return min(max(len(typed), len(source)), limit + 1)
    else:
        if typed[0] == source[0]:
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """
    if limit == 0 and typed != source:  # break the operation when reach the limit
        return 1
    elif limit == 0 and typed == source:
        return 0
    elif len(typed) == 0 or len(source) == 0:
...
def fastest_words(match):
    ...
    all_player_fastest_words = []
    for _ in player_indices:
        all_player_fastest_words.append([])

    for j in word_indices:
        time_for_word_temp = []
        for k in player_indices:
            time_for_word_temp.append(time(match, k, j))
        index = time_for_word_temp.index(min(time_for_word_temp))
        all_player_fastest_words[index].append(words[j])
    return all_player_fastest_words
    # END PROBLEM 10
