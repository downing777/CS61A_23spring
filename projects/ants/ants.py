...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        if self.exit: # consider the case that this place has no exit(start place)
...
class Ant(Insect):
    ...
    def __init__(self, health=1):
        ...
        super().__init__(health)  # inherit the father's property if you want to use the init method
    ...
    # no explicit class instance is required to call this kind of method
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b 处理包含/属性问题，不用加insect
            ant_orig = place.ant
            if ant_orig.can_contain(self):
                ant_orig.store_ant(self)
            elif self.can_contain(ant_orig):
                self.store_ant(ant_orig)
                place.ant = self
            else:
                assert place.ant is None, 'Two ants in {0}'.format(place)
            # END Problem 8b
        Insect.add_to(self, place)# 直接调用super class的函数，self指自己 ？ 存疑
...
class HarvesterAnt(Ant):
    ...
    name = 'Harvester'
    implemented = True
    # OVERRIDE CLASS ATTRIBUTES HERE(覆盖掉原类的attribute)
    food_cost = 2

    # 不额外添加 instance property, 无init 使用，不需要super.init 方法来input继承
    def action(self, gamestate):
...
class ThrowerAnt(Ant):
    ...
    food_cost = 3
    lower_bound = 0
    upper_bound = float('inf') # no upper restraint
    def nearest_bee(self):
        ...
        search_distance = 0
        while True:
            if not search_place.is_hive:
                if search_distance < self.lower_bound:
                    search_place = search_place.entrance
                    search_distance += 1
                elif search_distance > self.upper_bound:
                    return
                else:
                    if search_place.bees:
...
class FireAnt(Ant):
    ...
    def __init__(self, health=3):
        ...
        super().__init__(health) # double inheritance
    ...
    def reduce_health(self, amount):
        ...
        if amount < self.health:
            for bee in self.place.bees[:]:
               bee.reduce_health(amount) # it is not simply the remove the bee from the list, you should call the corresponding function to realize the effect
            Ant.reduce_health(self, amount) # will cause no resursive problem..?
...
class HungryAnt(Ant):
    ...
    def __init__(self, health = 1):
        ...
        # each hungry ant chews independently
...
class TankAnt(BodyguardAnt):
    ...
    implemented = True
    def __init__(self, health = 2):
        super().__init__(health) # make sure the creating forms are the same
    #__init__不是class函数，需要实例来调用，super（）
    def action(self, gamestate):
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        while search_place:
            if search_place.ant:
                search_place.ant.double()
                if search_place.ant.is_container and search_place.ant.ant_contained:
                    search_place.ant.ant_contained.double()
            ...
            def slow_action(gamestate):
                ...
                if target.slow_turns > 0:
                    ...
                    if gamestate.time % 2 == 0:
                        Bee.action(target, gamestate) ## ??? it's so weird, check tomorrow
                else:
