def flatten(s):
    ...
    for i in range(len(s)):
        if type(s[i]) != list:
            flattened_list.append(s[i])
        else:
            flattened_list = flattened_list + flatten(s[i])
...
def my_reduce(combiner, seq):
    ...
    if len(seq) == 1:
        ...
        start_new = combiner(seq[0], seq[1])
        return my_reduce(combiner, [start_new]+seq[2:])
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    benchmark = make_city('Benchmark', lat, lon)
    dis1, dis2 = distance(benchmark, city_a), distance(benchmark, city_b)
    if dis1 < dis2:
...
def count_palindromes(L):
    ...
    2
    """
    return len([i for i in range(len(L)) if L[i].lower() == L[i].lower()[::-1]])
