...
def partial_reverse(lst, start):
    ...
    """
    "*** YOUR CODE HERE ***"
    # start is the start index, not the element
    for i in range(len(lst) - 1, start-1, -1):
...
def pizza_sort_helper(lst, start):
    if start < len(lst)-1: # current index is not the last element
