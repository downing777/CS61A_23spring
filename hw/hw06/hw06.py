...
class VendingMachine:
    ...
    def add_funds(self, quantity):
        if self.stock == 0:
            return f'Nothing left to vend. Please restock. Here is your ${quantity}.'
        else:
...
class Game:
    ...
    def play(self):
        while not self.game_over():
            "*** YOUR CODE HERE ***"
            self.p1.choose(self.p2)(self.p2) # choose here is not completed which may cause the weird use
            self.p1, self.p2 = self.p2, self.p1 # cross assignment
        return self.winner()
...
class VirFib():
    ...
    def next(self):
        ...
        if self.value == 0:
            next_fib = VirFib(1)
            next_fib.previous = 0 # Keep track of the previous number by setting a new instance attribute, You can create new instance attributes for objects at any point, even outside the __init__ method.
            next_fib.__repr__()
            return next_fib
        else:
